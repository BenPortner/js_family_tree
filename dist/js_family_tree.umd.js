    ...
    class D3Renderer {
      ...
      <table style="margin-top: 2.5px;">
        ...
        render(layoutResult) {
            ...
            const svgWidth = Math.max(this.svg.node().clientWidth * 0.99, layoutResult.width);
            const svgHeight = Math.max(this.svg.node().clientHeight * 0.99, layoutResult.height);
            this.svg.attr('width', svgWidth).attr('height', svgHeight);
            // render links first so that they are behind the nodes
            this.renderLinks(layoutResult.graph);
